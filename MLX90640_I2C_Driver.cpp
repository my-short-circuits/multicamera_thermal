...
int MLX90640_I2CRead(uint8_t _deviceAddress, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data)
{
  ...
  while (bytesRemaining > 0)
  {
    ...
    if (numberOfBytesToRead & 1) numberOfBytesToRead--;
    if (numberOfBytesToRead == 0) return (-1);

    size_t bytesRead = Wire1.requestFrom((uint8_t)_deviceAddress,
                                         (size_t)numberOfBytesToRead);
    if (bytesRead != numberOfBytesToRead)
    {
      Serial.printf("Short MLX read: got %u of %u\n",
                    (unsigned)bytesRead, (unsigned)numberOfBytesToRead);
      return (-1);
    }
    ...
    for (uint16_t x = 0 ; x < numberOfBytesToRead / 2; x++)
    {
      ...
      {
        Serial.println("MLX read underrun");
        return (-1);
      }
...
int MLX90640_I2CWrite(uint8_t _deviceAddress, unsigned int writeAddress, uint16_t data)
{
  ...
  uint16_t dataCheck;
  if (MLX90640_I2CRead(_deviceAddress, writeAddress, 1, &dataCheck) != 0)
  {
    return -1;
  }
  if (dataCheck != data)
